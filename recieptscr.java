...
import javax.swing.*;
import java.sql.*;
public class recieptscr extends JFrame implements ActionListener{
	JLabel icode,iname,rate,qoh,dor,qtyrecieved;
	JTextField txtiname,txtrate,txtqoh,txtdor,txtqtyrecieved;
	JComboBox txticode;
	JButton b1,b2,b3;
	JMenuBar mbar;
	JMenu m1,m2,m3,m4;
	...
	public recieptscr(){
		...
		dor= new JLabel("dor");
		txtdor= new JTextField(50);
		qtyrecieved= new JLabel("quantity received");
		txtqtyrecieved= new JTextField(50);
		...
		dor.setBounds(50,330,60,50);
		txtdor.setBounds(130,330,120,50);
		qtyrecieved.setBounds(50,400,100,50);
		txtqtyrecieved.setBounds(150,400,120,50);
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(ae.getSource()== b1){
			...
			Connection con = DriverManager.getConnection("jdbc:odbc:inventorydsn");
			Statement stmt = con.createStatement();
			String query1 = "insert into tblreceipt(icode,dor,qtyreceived) values("+ txticode.getSelectedItem().toString() + ",'" + txtdor.getText() + "'," + txtqtyrecieved.getText() + ")";
			int x= stmt.executeUpdate(query1);
			String query2 = "update tblstock set qoh= qoh+ " + txtqtyrecieved.getText() + " where icode=" + txticode.getSelectedItem().toString();
			int y= stmt.executeUpdate(query2);
			JOptionPane.showMessageDialog(null,"quantity received and stock table updated");
			con.close();
			}
			catch(Exception e){
				JOptionPane.showMessageDialog(null,"Exception caught: " + e);
